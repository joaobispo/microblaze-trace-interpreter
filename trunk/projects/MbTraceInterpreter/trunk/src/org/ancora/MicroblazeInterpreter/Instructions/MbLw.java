...
/**
 *  Implements the MicroBlaze Load Word.
 * 
 * <p> Includes the instruction lhu .
 *
 * @author Joao Bispo
 */
public class MbLw implements Instruction, Builder {
    ...
    public Instruction build(TraceData data, MicroBlazeProcessor processor) {
        return new MbLw(data, processor);
    }
     ...
     */
    public MbLw(TraceData data, MicroBlazeProcessor processor) {
    ...
    public void execute() {
        ...
        if(bit0 == 0 & bit1 == 0) {
            rD = memory.loadWord(wordAddress);
        } else {
             System.out.println("lw: address is not word aligned ("+
                    wordAddress+"). Returning 0");
            rD = 0;
        }
