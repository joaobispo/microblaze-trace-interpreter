...
/**
 *  Implements the MicroBlaze Load Word Immediate.
 * 
 * <p> Includes the instruction lhui.
 *
 * @author Joao Bispo
 */
public class MbLwi implements Instruction, Builder {
    ...
    public Instruction build(TraceData data, MicroBlazeProcessor processor) {
        return new MbLwi(data, processor);
    }
     ...
     */
    public MbLwi(TraceData data, MicroBlazeProcessor processor) {
    ...
    public void execute() {
        ...
        if(bit0 == 0 & bit1 == 0) {
            rD = memory.loadHalfword(wordAddress);

        } else {
            System.out.println("lwi: address is not word aligned ("+
