...
/**
 *  Implements the MicroBlaze Store Word Immediate.
 * 
 * <p> Includes the instruction swi.
 *
 * @author Joao Bispo
 */
public class MbSwi implements Instruction, Builder {
    ...
    public Instruction build(TraceData data, MicroBlazeProcessor processor) {
        return new MbSwi(data, processor);
    }
     ...
     */
    public MbSwi(TraceData data, MicroBlazeProcessor processor) {
    ...
    public void execute() {
        ...
        int wordAddress = rA + immediate;

        int bit0 = BitOperations.getBit(0, wordAddress);
        int bit1 = BitOperations.getBit(1, wordAddress);
        if(bit0 == 0 & bit1 == 0) {
            memory.storeHalfword(wordAddress, rD);
        } else {
             System.out.println("swi: address is not word aligned ("+
                    wordAddress+"). Memory is unchanged.");
        }
