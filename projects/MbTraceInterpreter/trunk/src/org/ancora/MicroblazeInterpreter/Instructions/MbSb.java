...
/**
 *  Implements the MicroBlaze Store Byte.
 * 
 * <p> Includes the instruction sb.
 *
 * @author Joao Bispo
 */
public class MbSb implements Instruction, Builder {
    ...
    public Instruction build(TraceData data, MicroBlazeProcessor processor) {
        return new MbSb(data, processor);
    }
     ...
     */
    public MbSb(TraceData data, MicroBlazeProcessor processor) {
    ...
    public void execute() {
        ...
        int byteAddress = rA + rB;
        int rD = regs.read(regD);

        // Store byte
        memory.storeByte(byteAddress, rD);
